    ...
    public void discrete() {
        ...
        for (int Y = 1; Y <= mesh.getNY(); ++Y) {
            for (int X = 1; X <= mesh.getNX(); ++X) {
                ...
                mup = (mu.getFI()[IJ] + mut.getFI()[IJ] / KEpsilonModel.sigmak);
                muw = (mu.getFI()[IJW] + mut.getFI()[IJW] / KEpsilonModel.sigmak);
                mue = (mu.getFI()[IJE] + mut.getFI()[IJE] / KEpsilonModel.sigmak);
                mus = (mu.getFI()[IJS] + mut.getFI()[IJS] / KEpsilonModel.sigmak);
                mun = (mu.getFI()[IJN] + mut.getFI()[IJN] / KEpsilonModel.sigmak);
                ...
                //setting add source.
                Sp = -rho.getFI()[IJ] * epsilon.getFI()[IJ] / k.getFI()[IJ];
                Spad
                ...
                if (flag.atBoundary()) {
                    ...
                    double tau = 0;
                    double CK = KEpsilonModel.getCK(k.getFI()[IJ]);
