 ...
 */
package com.cup.tur.model.sstkomega.tf.gamma;
...
import com.cup.mesh.factory.BiSameV3;
...
//
    ...
    public SSTKOmegaTF(String position, String Title) {
    ...
    public void application(double dpdz, double hl, double R, int I)
    ...
    public void createGrid() {
        bm = new BiSameV3(12, Math.PI, Nx, Ny);
        bm.radia = R;
...
//        
        DOMEGA = new double[bm.numPx()][bm.numPy()][bm.numPz()];
        F1SST = new double[bm.numPx()][bm.numPy()][bm.numPz()];
//        
    ...
    double cacTur() {
        ...
        cacK(
                ...
                pro[mu].getF(), pro[rho].getF(), pro[mut].getF(), pro[S].getF(),
                sys
        );
        ...
        for (int Z = 1; Z <= mesh.getNZ(); ++Z) {
            for (int Y = 1; Y <= mesh.getNY(); ++Y) {
                for (int X = 1; X <= mesh.getNX(); ++X) {
                    double F2 = Turbulence.getF2(
                            F[K].getNewField()[X][Y][Z],
                            F[Omega].getNewField()[X][Y][Z],
                            pro[mu].getF()[X][Y][Z],
                            pro[rho].getF()[X][Y][Z],
                            bm.DN[X][Y][Z].getDistance()
                    );
                    pro[mut].getF()[X][Y][Z]
                            = cacMut(
                                    pro[rho].getF()[X][Y][Z],
                                    F[K].getNewField()[X][Y][Z],
                                    F[Omega].getNewField()[X][Y][Z],
                                    Math.sqrt(pro[S].getF()[X][Y][Z]),
                                    F2
                            );
                    pro[mu].getF()[X][Y][Z]
                            = setMu(phi[X][Y][Z],
                                    F[W].getNewField()[X][Y][Z],
                                    bm.cv[X][Y][Z].y);
                }
            }
        }
    ...
    double setMu(double phi, double U, double y) {
        double temp;
        double mu = 0.5 * (1 + phi) * testW[1] + 0.5 * (1 - phi) * testO[1];
        double B = y > 0 ? 100 : 50;
        if ((abs(y) / R) < 0.3) {
            temp = mu * (1.0 - 2 * U * U * B * log((abs(y) / R)));
        } else {
            temp = mu;
        }

//        temp = mu * (1.0 + 0.5 * U * U + B / (abs(y) / R));
        return temp;
    }
    ...
    void updateBound() {
        ...
        for (int Z = 0; Z <= bm.getNZ() + 1; Z++) {
            for (int Y = 0; Y <= bm.getNY() + 1; Y++) {
                value = F[Omega].getNewField()[xw + 1][Y][Z];
                F[Omega].setNewField(xw, Y, Z, value);
                //xe
                value = F[Omega].getNewField()[xe - 1][Y][Z];
                F[Omega].setNewField(xe, Y, Z, value);
            }
        }
        //Y边界
        for (int Z = 0; Z <= bm.getNZ() + 1; Z++) {
            for (int X = 0; X <= bm.getNX() + 1; X++) {
                value = F[Omega].getNewField()[X][yw + 1][Z];
                F[Omega].setNewField(X, yw, Z, value);
                //ye
                value = F[Omega].getNewField()[X][ye - 1][Z];
                F[Omega].setNewField(X, ye, Z, value);
            }
        }
...
//                  设定湍流粘度
    ...
    /**
     * FIND F1SST, ALPHASST, BETASST, DOMEGA.
     */
    void SST_BLENDING_FUNC() {
        //计算K和Omega的梯度乘积
        cacDKW(F[K], F[Omega], mesh, DOMEGA);
        //计算混合因子
        for (int Z = 1; Z <= mesh.getNZ(); ++Z) {
            for (int Y = 1; Y <= mesh.getNY(); ++Y) {
                for (int X = 1; X <= mesh.getNX(); ++X) {
                    //混合因子求解
                    double DKW = DOMEGA[X][Y][Z];
                    double CDKW = Turbulence.getCDKW(
                            pro[rho].getF()[X][Y][Z],
                            F[Omega].getNewField()[X][Y][Z],
                            DKW
                    );
                    double KSI = Turbulence.getKSI(
                            F[K].getNewField()[X][Y][Z],
                            F[Omega].getNewField()[X][Y][Z],
                            pro[mu].getF()[X][Y][Z],
                            pro[rho].getF()[X][Y][Z],
                    ...
                    F1SST[X][Y][Z] = Turbulence.getF1(KSI);
                }
            }
        }
     ...
     * 计算交叉项的导数部分
     */
    void cacDKW(Field K, Field Omega, Mesh mesh, double[][][] S) {
        double alpha;
        double a, b, c, d;
        for (int Z = 1; Z <= mesh.getNZ(); ++Z) {
            for (int Y = 1; Y <= mesh.getNY(); ++Y) {
                for (int X = 1; X <= mesh.getNX(); ++X) {
                    ...
                    a = (K.getNewField()[X + 1][Y][Z] - K.getNewField()[X - 1][Y][Z])
                            / (mesh.getDXP()[X - 1] + mesh.getDXP()[X]);
                    b = (Omega.getNewField()[X + 1][Y][Z] - Omega.getNewField()[X - 1][Y][Z])
                            / (mesh.getDXP()[X - 1] + mesh.getDXP()[X]);
                    c = (K.getNewField()[X][Y + 1][Z] - K.getNewField()[X][Y - 1][Z])
                            / (mesh.getDYP()[Y - 1] + mesh.getDYP()[Y]);
                    d = (Omega.getNewField()[X][Y + 1][Z] - Omega.getNewField()[X][Y - 1][Z])
                            / (mesh.getDYP()[Y - 1] + mesh.getDYP()[Y]);
                    S[X][Y][Z] = (a * b + c * d) / alpha;
                }
            }
        }
    }

    final double alpha1 = 5.0 / 9.0;
    final double alpha2 = 0.44;
    final double beta1 = 3.0 / 40.0;
    final double beta2 = 0.0828;
    final double sigmak1 = 0.85;
    final double sigmak2 = 1.0;
    final double sigmaw1 = 0.5;
    final double sigmaw2 = 0.856;
    final double a1 = 0.31;
    final double betastar = 0.09;
    final double CMU = 0.09;
    final double CMU25 = sqrt(sqrt(CMU));

    double getPhi(double phi1, double phi2, double F1) {
        ...
        for (int Z = 1; Z <= mesh.getNZ(); ++Z) {
            for (int Y = 1; Y <= mesh.getNY(); ++Y) {
                for (int X = 1; X <= mesh.getNX(); ++X) {
                    ...
                    Fn = 0;
//                    
                    sigma = getPhi(sigmak1, sigmak2, F1SST[X][Y][Z]);
                    mup = (mu[X][Y][Z] + sigma * Mut[X][Y][Z]);
                    muw = (mu[X - 1][Y][Z] + sigma * Mut[X - 1][Y][Z]);
                    mue = (mu[X + 1][Y][Z] + sigma * Mut[X + 1][Y][Z]);
                    mus = (mu[X][Y - 1][Z] + sigma * Mut[X][Y - 1][Z]);
                    mun = (mu[X][Y + 1][Z] + sigma * Mut[X][Y + 1][Z]);
//
                    gamw = faceValue(muw, mup - muw, mesh.gettPx()[X - 1], mesh.getTx()[X - 1], mesh.gettPx()[X]);
                    game = faceValue(mup, mue - mup, mesh.gettPx()[X], mesh.getTx()[X], mesh.gettPx()[X + 1]);
                    gams = faceValue(mus, mup - mus, mesh.gettPy()[Y - 1], mesh.getTy()[Y - 1], mesh.gettPy()[Y]);
...
//                    
                    ...
                    if ((flag.w == 1) || (flag.e == 1) || (flag.s == 1) || (flag.n == 1)) {
                        ...
                        GEN = Math.min(
                                (Mut[X][Y][Z] * S[X][Y][Z]),
                                10.0 * betastar * rho[X][Y][Z]
                                * Omega[X][Y][Z] * F.getOldField()[X][Y][Z]
                        );
                    }
                    //</editor-fold>
//
        ...
        for (int Z = 1; Z <= mesh.getNZ(); ++Z) {
            for (int Y = 1; Y <= mesh.getNY(); ++Y) {
                for (int X = 1; X <= mesh.getNX(); ++X) {
                    ...
                    Fn = 0;
//                  
                    sigma = getPhi(sigmaw1, sigmaw2, F1SST[X][Y][Z]);
                    mup = (mu[X][Y][Z] + sigma * Mut[X][Y][Z]);
                    muw = (mu[X - 1][Y][Z] + sigma * Mut[X - 1][Y][Z]);
                    mue = (mu[X + 1][Y][Z] + sigma * Mut[X + 1][Y][Z]);
                    mus = (mu[X][Y - 1][Z] + sigma * Mut[X][Y - 1][Z]);
...
//                    
                    ...
                    coeW.an[X][Y][Z] = (1 - flag.n) * (Math.max(-Fn, 0.0) + Dn);//n
//               
                    beta = getPhi(beta1, beta2, F1SST[X][Y][Z]);
...
//得到ap系数
                    coeW.ap[X][Y][Z]
                            ...
                            + (Fe - Fw) + (Fn - Fs) - (Sp + Spad) * vol;
//                    
                    alpha = getPhi(alpha1, alpha2, F1SST[X][Y][Z]);
                    Sc
                            = alpha * (rho[X][Y][Z] * S[X][Y][Z])
                            + 2.0 * (1 - F1SST[X][Y][Z]) * sigmaw2 * rho[X][Y][Z]
                            / F.getOldField()[X][Y][Z] * DOMEGA[X][Y][Z];
                    Scad
                            = Scadw(F.getBoundary().getType(X - 1, Y, Z),
                                    Dw, Fw, mesh.getDXP()[X - 1], vol,
                                    F.getBoundary().getBoundaryRegion().parameter[X - 1][Y][Z])
            ...
            for (int Y = 1; Y <= mesh.getNY(); ++Y) {
                for (int X = 1; X <= mesh.getNX(); ++X) {
                    ...
                    if ((flag.w == 1) || (flag.e == 1) || (flag.s == 1) || (flag.n == 1)) {
                        ...
                        coeW.as[X][Y][Z] = 0;//s
                        coeW.an[X][Y][Z] = 0;//n
                        coeW.b[X][Y][Z] = sqrt(WLOG * WLOG + WVIS * WVIS);
                    }
                    //under-relaxization  
                    coeW.b[X][Y][Z]
                            = coeW.b[X][Y][Z] + (URFFI - 1.0) * coeW.ap[X][Y][Z]
...
//求解
    ...
    void RESULT(int count) throws FileNotFoundException {
        ...
        String var = "Variables=\"X\",\"Y\",\"DN\",\"W\",\"K\",\"Omega\",\"mut\",\"mu\",\"T\",\"S\",\"phi\"";
...
//        
        ...
        for (int J = 0; J <= mesh.getNY() + 1; ++J) {
            for (int I = 0; I <= mesh.getNX() + 1; ++I) {
                fileWriter.printf("%16.6E\t", bm.cv[I][J][K].x);
                fileWriter.printf("%16.6E\t", bm.cv[I][J][K].y);
                fileWriter.printf("%16.6E\t", bm.DN[I][J][K].getDistance());
                fileWriter.printf("%16.6E\t", F[W].getNewField()[I][J][K]);
                fileWriter.printf("%16.6E\t", F[this.K].getNewField()[I][J][K]);
                fileWriter.printf("%16.6E\t", F[Omega].getNewField()[I][J][K]);
                fileWriter.printf("%16.6E\t", pro[mut].getF()[I][J][K]);
                fileWriter.printf("%16.6E\t", pro[mu].getF()[I][J][K]);
    ...
    public double getGcwater() {
        return Gcwater;
    }

    public double getGcoil() {
        return Gcoil;
    }

    public double F(double Gwater) {
        return this.Gcwater - Gwater;
    }

    public double G(double Goil) {
        return this.Gcoil - Goil;
    }
}
