 ...
 */
package com.cup.tur.model.kepsilon.tf;
...
import static com.cup.turbulence.Turbulence.setEpsilon;
...
//
    ...
    final double CMU25 = Math.sqrt(Math.sqrt(CMU));
    final double Ce1 = 1.44;
    final double Ce2 = 1.92;
    final double sigmak = 1.0;
    final double sigmae = 1.3;
    final double CTRANS = 11.63;

    public KEpsilonTF(String position, String Title) {
    ...
    public void createGrid() {
        bm = new BiSameV2(12, Math.PI, Nx, Ny);
        bm.radia = R;
    ...
    void setBoundary() {
        ...
        for (int Z = 0; Z <= bm.getNZ() + 1; Z++) {
            for (int Y = 0; Y <= bm.getNY() + 1; Y++) {
                ...
                F[Epsilon].getBoundary().getBoundaryRegion().setType(xw, Y, Z, 1, bc[Epsilon]);
                ...
                F[Epsilon].getBoundary().getBoundaryRegion().setType(xe, Y, Z, 1, bc[Epsilon]);
        ...
        for (int Z = 0; Z <= bm.getNZ() + 1; Z++) {
            for (int X = 0; X <= bm.getNX() + 1; X++) {
                ...
                F[Epsilon].getBoundary().getBoundaryRegion().setType(X, yw, Z, 1, bc[Epsilon]);
                ...
                F[Epsilon].getBoundary().getBoundaryRegion().setType(X, ye, Z, 1, bc[Epsilon]);
    ...
    double cacTur() {
        ...
        cacK(mesh, F[K],
                F[Epsilon].getNewField(), F[W].getNewField(),
        ...
        cacEpsilon(mesh, F[Epsilon],
                F[K].getNewField(), F[W].getNewField(),
                pro[mu].getF(), pro[rho].getF(), pro[mut].getF(), pro[S].getF(), sys);
        RES[Epsilon] = getRes(F[Epsilon].getNewField(), F[Epsilon].getOldField());
        F[Epsilon].copyNew2Old();
        return RES[Epsilon];
    }
...
//                    
                    value = setEpsilon(F[K].getNewField()[X][Y][Z], R);
                    F[Epsilon].setNewField(X, Y, Z, value);
//                    
                    pro[mut].getF()[X][Y][Z]
                            = cacMut(
                                    pro[rho].getF()[X][Y][Z],
    ...
    public double cacMut(double rho, double K, double epsilon) {
        double temp = CMU * rho * K * K / (epsilon + 1e-30);
        return temp;
    }
    ...
    double getMaxRES() {
        ...
        System.out.println("RESOmega = " + RES[Epsilon]);
        ...
        for (int Z = 1; Z <= mesh.getNZ(); ++Z) {
            for (int Y = 1; Y <= mesh.getNY(); ++Y) {
                for (int X = 1; X <= mesh.getNX(); ++X) {
                    ...
                    Fn = 0;
//                    
                    mup = (mu[X][Y][Z] + Mut[X][Y][Z] / sigmak);
                    muw = (mu[X - 1][Y][Z] + Mut[X - 1][Y][Z] / sigmak);
                    mue = (mu[X + 1][Y][Z] + Mut[X + 1][Y][Z] / sigmak);
                    mus = (mu[X][Y - 1][Z] + Mut[X][Y - 1][Z] / sigmak);
                    mun = (mu[X][Y + 1][Z] + Mut[X][Y + 1][Z] / sigmak);
//
                    gamw = faceValue(muw, mup - muw, mesh.gettPx()[X - 1], mesh.getTx()[X - 1], mesh.gettPx()[X]);
                    game = faceValue(mup, mue - mup, mesh.gettPx()[X], mesh.getTx()[X], mesh.gettPx()[X + 1]);
                    gams = faceValue(mus, mup - mus, mesh.gettPy()[Y - 1], mesh.getTy()[Y - 1], mesh.gettPy()[Y]);
...
//                    
                    ...
                    coeW.an[X][Y][Z] = (1 - flag.n) * (Math.max(-Fn, 0.0) + Dn);//n
//
                    Sp = -rho[X][Y][Z] * Omega[X][Y][Z] / F.getOldField()[X][Y][Z];
                    Spad
                            = Spadw(F.getBoundary().getType(X - 1, Y, Z),
                                    Dw, Fw, mesh.getDXP()[X - 1], vol,
                                    F.getBoundary().getBoundaryRegion().parameter[X - 1][Y][Z])
...
//
     ...
     * @param sys
     */
    void cacEpsilon(Mesh mesh, Field F,
            double[][][] K, double[][][] W,
            double[][][] mu, double[][][] rho,
            double[][][] Mut, double[][][] S,
            SystemControl sys) {
        double URF = SOR[Epsilon];
        double URFFI = 1. / URF;
        double Fe, Fw, Fn, Fs;
        double De, Dw, Dn, Ds;
        double Spad, Scad, Sp, Sc;
        ...
        for (int Z = 1; Z <= mesh.getNZ(); ++Z) {
            for (int Y = 1; Y <= mesh.getNY(); ++Y) {
                for (int X = 1; X <= mesh.getNX(); ++X) {
                    ...
                    Fn = 0;
//                    
                    mup = (mu[X][Y][Z] + Mut[X][Y][Z] / sigmae);
                    muw = (mu[X - 1][Y][Z] + Mut[X - 1][Y][Z] / sigmae);
                    mue = (mu[X + 1][Y][Z] + Mut[X + 1][Y][Z] / sigmae);
                    mus = (mu[X][Y - 1][Z] + Mut[X][Y - 1][Z] / sigmae);
                    mun = (mu[X][Y + 1][Z] + Mut[X][Y + 1][Z] / sigmae);
//                    
                    gamw = faceValue(muw, mup - muw, mesh.gettPx()[X - 1], mesh.getTx()[X - 1], mesh.gettPx()[X]);
                    game = faceValue(mup, mue - mup, mesh.gettPx()[X], mesh.getTx()[X], mesh.gettPx()[X + 1]);
                    gams = faceValue(mus, mup - mus, mesh.gettPy()[Y - 1], mesh.getTy()[Y - 1], mesh.gettPy()[Y]);
...
//求解
    ...
    void RESULT(int count) throws FileNotFoundException {
        ...
        String var = "Variables=\"X\",\"Y\",\"W\",\"K\",\"Omega\",\"T\",\"S\",\"phi\"";
...
//        
        ...
        for (int J = 0; J <= mesh.getNY() + 1; ++J) {
            for (int I = 0; I <= mesh.getNX() + 1; ++I) {
                ...
                fileWriter.printf("%16.6E\t", F[Epsilon].getNewField()[I][J][K]);
