...
public class KW implements Cloneable {
    ...
    double R, g = 0;
    double[][][] phi;
    ProF[] pro = new ProF[6], proW = new ProF[6], proO = new ProF[6];
    int S = 0, rho = 1, mu = 2, mut = 3, lambda = 4, cp = 5;
    Field[] F = new Field[4], FW = new Field[4], FO = new Field[4];
    Coefficient[] coe = new Coefficient[4], coeW = new Coefficient[4], coeO = new Coefficient[4];
    ...
    void setTimeConvection(KW xold, KW x_1old, double dt, double dz) {
        this.Xold = xold;
        this.X_1old = x_1old;
        this.dz = dz;
        if (dt != 0) {
    ...
    void createFields() {
        ...
        for (int i = 0; i < coeW.length; i++) {
            coe[i] = new Coefficient(bm.numPx(), bm.numPy(), bm.numPz());
    ...
    void setBoundary() {
        ...
        for (int Z = 0; Z <= bm.getNZ() + 1; Z++) {
            for (int Y = 0; Y <= bm.getNY() + 1; Y++) {
                ...
                F[W].getBoundary().getBoundaryRegion().setType(xw, Y, Z, 1, temp[W]);
                temp[K] = new RobinBC(0.0, 1.0, 0.0, 1.0);
                F[K].getBoundary().getBoundaryRegion().setType(xw, Y, Z, 1, temp[K]);
                temp[Omega] = new RobinBC(0.0, 1.0, 0.0, 1.0);
                F[Omega].getBoundary().getBoundaryRegion().setType(xw, Y, Z, 1, temp[Omega]);
                temp[T] = new RobinBC(59.0, 15.0, 375.0, 1.0);
                F[T].getBoundary().getBoundaryRegion().setType(xw, Y, Z, 3, bc);
                //xe
                temp[W] = new RobinBC(0.0, 1.0, 0.0, 1.0);
                F[W].getBoundary().getBoundaryRegion().setType(xe, Y, Z, 1, temp[W]);
                temp[K] = new RobinBC(0.0, 1.0, 0.0, 1.0);
                F[K].getBoundary().getBoundaryRegion().setType(xe, Y, Z, 1, temp[K]);
                temp[Omega] = new RobinBC(0.0, 1.0, 0.0, 1.0);
                F[Omega].getBoundary().getBoundaryRegion().setType(xe, Y, Z, 1, temp[Omega]);
                temp[T] = new RobinBC(59.0, 15.0, 375.0, 1.0);
                F[T].getBoundary().getBoundaryRegion().setType(xe, Y, Z, 3, bc);
            }
        }
        //Y边界
        for (int Z = 0; Z <= bm.getNZ() + 1; Z++) {
            for (int X = 0; X <= bm.getNX() + 1; X++) {
                temp[W] = new RobinBC(0.0, 1.0, 0.0, 1.0);
                F[W].getBoundary().getBoundaryRegion().setType(X, yw, Z, 1, temp[W]);
                temp[K] = new RobinBC(0.0, 1.0, 0.0, 1.0);
                F[K].getBoundary().getBoundaryRegion().setType(X, yw, Z, 1, temp[K]);
                temp[Omega] = new RobinBC(0.0, 1.0, 0.0, 1.0);
                F[Omega].getBoundary().getBoundaryRegion().setType(X, yw, Z, 1, temp[Omega]);
                temp[T] = new RobinBC(59.0, 15.0, 375.0, 1.0);
                F[T].getBoundary().getBoundaryRegion().setType(X, yw, Z, 3, bc);
                //ye
                temp[W] = new RobinBC(0.0, 1.0, 0.0, 1.0);
                F[W].getBoundary().getBoundaryRegion().setType(X, ye, Z, 1, temp[W]);
                temp[K] = new RobinBC(0.0, 1.0, 0.0, 1.0);
                F[K].getBoundary().getBoundaryRegion().setType(X, ye, Z, 1, temp[K]);
                temp[Omega] = new RobinBC(0.0, 1.0, 0.0, 1.0);
                F[Omega].getBoundary().getBoundaryRegion().setType(X, ye, Z, 1, temp[Omega]);
                temp[T] = new RobinBC(59.0, 15.0, 375.0, 1.0);
                F[T].getBoundary().getBoundaryRegion().setType(X, ye, Z, 3, bc);
            }
        }
        //</editor-fold>
        //WATER
        xw = 0;
        xe = WaterMesh.getNX() + 1;
        yw = 0;
        ye = WaterMesh.getNY() + 1;
        //X边界
        //<editor-fold>
...
//        进入大循环
     ...
     * 计算湍流动能场
     */
    void cacK(Mesh mesh, Field K, Field Omega,
            double[][][] mu, double[][][] rho,
            double[][][] Mut, double[][][] S,
            SystemControl sys
    ) {
        double URF = SOR[this.K];
        double URFFI = 1. / URF;
        double Fe, Fw, Fn, Fs;
        double De, Dw, Dn, Ds;
        ...
        int i = mesh.numPx();
        int j = mesh.numPy();
        Coefficient coeK = new Coefficient(i, j);
...
//设定附加源项
                    ...
                    Spad
                            = Spadw(K.getBoundary().getType(X - 1, Y, Z),
                                    Dw, Fw, mesh.getDXP()[X - 1], vol,
                                    K.getBoundary().getBoundaryRegion().parameter[X - 1][Y][Z])
                            + Spade(K.getBoundary().getType(X + 1, Y, Z),
                                    De, Fe, mesh.getDXP()[X], vol,
                                    K.getBoundary().getBoundaryRegion().parameter[X + 1][Y][Z])
                            + Spadw(K.getBoundary().getType(X, Y - 1, Z),
                                    Ds, Fs, mesh.getDYP()[Y - 1], vol,
                                    K.getBoundary().getBoundaryRegion().parameter[X][Y - 1][Z])
                            + Spade(K.getBoundary().getType(X, Y + 1, Z),
...
//                    得到源项
                    ...
                    Scad
                            = Scadw(K.getBoundary().getType(X - 1, Y, Z),
                                    Dw, Fw, mesh.getDXP()[X - 1], vol,
                                    K.getBoundary().getBoundaryRegion().parameter[X - 1][Y][Z])
                            + Scade(K.getBoundary().getType(X + 1, Y, Z),
                                    De, Fe, mesh.getDXP()[X], vol,
                                    K.getBoundary().getBoundaryRegion().parameter[X + 1][Y][Z])
                            + Scadw(K.getBoundary().getType(X, Y - 1, Z),
                                    Ds, Fs, mesh.getDYP()[Y - 1], vol,
                                    K.getBoundary().getBoundaryRegion().parameter[X][Y - 1][Z])
                            + Scade(K.getBoundary().getType(X, Y + 1, Z),
                                    Dn, Fn, mesh.getDYP()[Y], vol,
                                    K.getBoundary().getBoundaryRegion().parameter[X][Y + 1][Z]);
...
//Under-relaxizaion
    ...
    public void setOmega() {
        for (int z = 1; z < F[Omega].getNewField()[0][0].length - 1; ++z) {
            for (int y = 1; y < F[Omega].getNewField()[0].length - 1; ++y) {
                for (int x = 1; x < F[Omega].getNewField().length - 1; ++x) {
                    double value
                            = pow(0.09, 0.75)
                            * pow(F[K].getNewField()[x][y][z], 3.0 / 2.0)
                            / Turbulence.turbulenceLengthscale(bm.radia * 2);
                    value = value / F[K].getNewField()[x][y][z];
...
//under-relaxization
    ...
    public KW(double Qso, double Qsw, double Radia) {
        this.Qso = Qso;
        this.Qsw = Qsw;
        this.R = Radia;
    }
    ...
    public KW(double Qso, double Qsw, double TOil, double TWater, double Radia) {
        this.Qso = Qso;
        this.Qsw = Qsw;
        this.TOil = TOil;
        this.TWater = TWater;
    ...
    public KW(double Qso, double Qsw, double TOil, double TWater, RobinBC BC, double Radia) {
        this.Qso = Qso;
        this.Qsw = Qsw;
        this.TOil = TOil;
        this.TWater = TWater;
...
//        
    ...
    @Override
    public KW clone() throws CloneNotSupportedException {
        KW clone = new KW();
