 ...
 * @author 齐雪宇
 */
public final class komegaTFOWAngeli2T {
...
//得到源项
     ...
     * @param dpdz
     */
    void cacW(double dpdz, Mesh mesh,
            Field W,
...
//      K水相
        for (int x = 1; x < FW[K].getNewField().length - 1; ++x) {
            leta = sqrt(WaterMesh.alpha(WaterMesh.gettPx()[x], WaterMesh.gettPy()[1]));
            yp = Tool.distance(
    ...
    public komegaTFOWAngeli2T(double Qso, double Qsw, double Radia) {
        this.Qso = Qso;
        this.Qsw = Qsw;
        this.R = Radia;
    }
...
//        double temp;
//        double alpha = 6.0;
//        temp = max(0, (1.0 - abs(mesh.gettPx()[x]) / R));
//        temp = exp(alpha * temp);
//        temp = alpha * (-temp * temp + 1);
        return 1.0;
    }
