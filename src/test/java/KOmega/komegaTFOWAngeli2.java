 ...
 * @author 齐雪宇
 */
public final class komegaTFOWAngeli2 {
    ...
    Coefficient coeKoil, coeOoil, coeToil;
    Coefficient coeKwater, coeOwater, coeTwater;
    double dpdz, hl;
    double Qso, Qsw;
    double R;
    double g = 0;
        ...
        for (int i = 0; i < OilMesh.numPx(); i++) {
            for (int j = 0; j < OilMesh.numPy(); j++) {
                if (i == 0 || i == (OilMesh.numPx() - 1)
                    ...
                    TOil.getBoundary().setType(i, j, k, 3);
                    if (j == (OilMesh.numPy() - 1)) {
        ...
        for (int i = 0; i < WaterMesh.numPx(); i++) {
            for (int j = 0; j < WaterMesh.numPy(); j++) {
                if (i == 0 || i == (WaterMesh.numPx() - 1)
                    ...
                    TWater.getBoundary().setType(i, j, k, 3);
                    if (j == 0) {
...
//under-relaxization
    ...
    public komegaTFOWAngeli2(double Qso, double Qsw, double Radia) {
        this.Qso = Qso;
        this.Qsw = Qsw;
        this.R = Radia;
    }
...
//        
        ...
        do {
            ...
            pw.flush();
            komegaTFOWAngeli2 F1 = new komegaTFOWAngeli2(Qoil, Qwater, Radia);
            ...
            pw.flush();
            komegaTFOWAngeli2 Fx1 = new komegaTFOWAngeli2(Qoil, Qwater, Radia);
            Fx1.application(xold * 1.01, yold);
            System.out.println("Fy1   " + " xold =" + xold + " yold * 1.01 =" + yold * 1.01);
            pw.printf("Fy1 xold =%e\t yold * 1.01 =%e\t\n", xold, yold * 1.01);
            pw.flush();
            komegaTFOWAngeli2 Fy1 = new komegaTFOWAngeli2(Qoil, Qwater, Radia);
...
//            
    ...
    private void cacTWater() {
        double URF = 0.8;
        double URFFI = 1. / URF;
        double Fe = 0, Fw = 0, Fn = 0, Fs = 0;
        double De, Dw, Dn, Ds;
        double Spad, Scad, Sp, Sc;
        double muw, mue, mus, mun, mup;
        double gamw, game, gams, gamn;
        ...
        for (int z = 1; z < TWater.getNewField()[0][0].length - 1; ++z) {
            for (int y = 1; y < TWater.getNewField()[0].length - 1; ++y) {
                for (int x = 1; x < TWater.getNewField().length - 1; ++x) {
                    ...
                    double vol = WaterMesh.getDXU()[x - 1] * WaterMesh.getDYV()[y - 1]
                            * WaterMesh.J(WaterMesh.gettPx()[x], WaterMesh.gettPy()[y]);
//
...
//得到ap系数
                    ...
                    Scad
                            ...
                            * Scad1(TWater.getBoundary().getType(x - 1, y, z),
                                    WaterMesh.getDYV()[y - 1], vol, WaterMesh.getDXP()[x - 1],
                                    gamw, Fw, TWater.getNewField()[x - 1][y][z], 0, 1, hf)
                            + flagT.e
                            * Scad2(TWater.getBoundary().getType(x + 1, y, z),
                                    WaterMesh.getDYV()[y - 1], vol, WaterMesh.getDXP()[x],
                                    game, Fe, TWater.getNewField()[x + 1][y][z], 0, 1, hf)
                            + flagT.s
                            * Scad1(TWater.getBoundary().getType(x, y - 1, z),
                                    WaterMesh.getDXU()[x - 1], vol, WaterMesh.getDYP()[y - 1],
                                    gams, Fs, TWater.getNewField()[x][y - 1][z], 0, 1, hf)
                            + flagT.n
                            * Scad2(TWater.getBoundary().getType(x, y + 1, z),
                                    WaterMesh.getDXU()[x - 1], vol, WaterMesh.getDYP()[y],
                                    gamn, Fn, TWater.getNewField()[x][y + 1][z], 0, 1, hf);
        ...
        for (int z = 1; z < TOil.getNewField()[0][0].length - 1; ++z) {
            for (int y = 1; y < TOil.getNewField()[0].length - 1; ++y) {
                for (int x = 1; x < TOil.getNewField().length - 1; ++x) {
                    ...
                    double vol = OilMesh.getDXU()[x - 1] * OilMesh.getDYV()[y - 1]
                            * OilMesh.J(OilMesh.gettPx()[x], OilMesh.gettPy()[y]);
//
...
//得到ap系数
                    ...
                    Scad
                            ...
                            * Scad1(TOil.getBoundary().getType(x - 1, y, z),
                                    OilMesh.getDYV()[y - 1], vol, OilMesh.getDXP()[x - 1],
                                    gamw, Fw, TOil.getNewField()[x - 1][y][z], 0, 1, hf)
                            + flagT.e
                            * Scad2(TOil.getBoundary().getType(x + 1, y, z),
                                    OilMesh.getDYV()[y - 1], vol, OilMesh.getDXP()[x],
                                    game, Fe, TOil.getNewField()[x + 1][y][z], 0, 1, hf)
                            + flagT.s
                            * Scad1(TOil.getBoundary().getType(x, y - 1, z),
                                    OilMesh.getDXU()[x - 1], vol, OilMesh.getDYP()[y - 1],
                                    gams, Fs, TOil.getNewField()[x][y - 1][z], 0, 1, hf)
                            + flagT.n
                            * Scad2(TOil.getBoundary().getType(x, y + 1, z),
                                    OilMesh.getDXU()[x - 1], vol, OilMesh.getDYP()[y],
                                    gamn, Fn, TOil.getNewField()[x][y + 1][z], 0, 1, hf);
    ...
    void updateInterfaceT() {
        for (int x = 1; x < W.getNewField().length - 1; ++x) {
            ...
            double yWater = Tool.distance(
                    ...
                    mesh.realX(WaterMesh.gettPx()[x], WaterMesh.gettPy()[0]),
                    mesh.realY(WaterMesh.gettPx()[x], WaterMesh.gettPy()[0]));
            double w = (lambdaWater[x][1][1]) / yWater;
            double o = (lambdaOil[x][OilMesh.getNY()][1]) / yOil;
            double u = (WOil.getNewField()[x][OilMesh.getNY()][1] * o + WWater.getNewField()[x][1][1] * w)
                    / (o + w);
            WOil.setNewField(x, OilMesh.getNY() + 1, 1, u);
            WWater.setNewField(x, 0, 1, u);
